...
class PatchMatch:
    ...
    def read_image(self, filename, key):
        success = False
        msg = 'ERROR: Image file with file name, {}, is NOT opened successfully!'.format(filename)
        ...
        img = cv.imread(filename) # keep 3-channal, otherwise, .png would have four dimensions
        ...
        if img is not None:
            ...
            if self._images.has_key(key):
                ...
                success = True
                msg = 'ERROR: Image file with file name, {}, opened and stored successfully!'.format(filename)
            # if key is not valid, reset the error message
            else:
                msg = 'ERROR: Invalid key, {}, given when read the image!'.format(key)
    ...
    def write_image(self, filename, key):
        success = False
        msg = 'ERROR: Result image with file name, {}, is NOT written successfully!'.format(filename)
        ...
        if self._images.has_key(key):
            ...
            if result_img is not None:
                ...
                if isWritten is not False:
                    success = True
                    msg = 'ERROR: Result image with file name, {}, is written successfully!'.format(filename)
            else:
                msg = 'ERROR: Result image with key, {}, is None when write the image!'.format(key)
        # if key is not valid, reset the error message
        else:
            msg = 'ERROR: Invalid key, {}, given when write the image!'.format(key)
