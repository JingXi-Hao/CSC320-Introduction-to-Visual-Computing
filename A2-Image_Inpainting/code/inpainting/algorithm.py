...
class Inpainting:
    ...
    def readImage(self, fileName, key):
        ...
        img = cv.imread(fileName, -1).astype(np.uint8)
        
        # if image exits and read successfully
        if img is not None:
            # if key is valid, then store the img
            if self._images.has_key(key):
                self._images[key] = img
                success = True
                msg = 'Image file with file name, {}, opened and stored successfully!'.format(fileName)
            # if key is not valid, reset the error message
            else:
                msg = 'Invalid key, {}, given when read the image!'.format(key)
    ...
    def writeImage(self, fileName, key):
        ...
        # if key is valid, then extract the corresponding image
        if self._images.has_key(key):
            result_img = self._images[key]
            # if result_img is valid
            if result_img is not None:
                result_img = np.rint(result_img).astype(np.uint8)
                # write the image with given name
                ...
                msg = 'Result image with key, {}, is None when write the image!'.format(key)
        # if key is not valid, reset the error message
        else:
            msg = 'Invalid key, {}, given when write the image!'.format(key)
    ...
    def exampleBasedInpainting(self, imviewer, maxIterations=None):
        ...
        # extract images with corresponding keys
        source = self._images['source']
        alpha = self._images['alpha']
        errorCount = 0
        newMsg = ''

        # check whether the input images are valid
        if source is None:
            newMsg = 'Source image is NOT valid!'
            errorCount += 1

        if alpha is None:
            newMsg = newMsg + ' Alpha image is NOT valid!'
            errorCount += 1
        ...
        # check whether source is 3-channel uint8 image
        if len(source.shape) != 3:
            newMsg = 'Source image is NOT 3-channel!'
            errorCount += 1

        if source.dtype != np.uint8:
            newMsg = newMsg + ' Source image is NOT uint8 data type!'
            errorCount += 1
        ...
        # check whether alpha is one-channel uint8 image
        if len(alpha.shape) != 2:
            newMsg = 'Alpha image is NOT one-channel!'
            errorCount += 1

        if alpha.dtype != np.uint8:
            newMsg = newMsg + ' Alpha image is NOT uint8 data type!'
            errorCount += 1
        ...
        # check whether alpha and source have same dimensions
        if (source.shape[0] != alpha.shape[0]) or (source.shape[1] != alpha.shape[1]):
            msg = 'Alpha image and source image do NOT have same dimensions!'
            return success, msg

        # all inputs are valid
        success = True
        msg = "All inputs and outputs are valid!"
